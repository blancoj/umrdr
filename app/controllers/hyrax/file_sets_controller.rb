  ...
  class FileSetsController < ApplicationController
   ...
   def create_from_upload(params)
      ...
      if !file
        ...
        render_json_response(response_type: :unprocessable_entity, options: { errors: { files: "#{file.original_filename} has no content! (Zero length file)" }, description: t('yyrax.api.unprocessable_entity.empty_file') })
      elsif too_large_file?(file)
        render_json_response(response_type: :unprocessable_entity, options: { errors: { files: "#{file.original_filename} is larger than #{Hyrax.config.max_file_size_str}" }, description: t('hyrax.api.unprocessable_entity.too_large_file') })
      else
        process_file(file)
      end
